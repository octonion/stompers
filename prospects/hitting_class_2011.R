...
player_model <- class ~ age + speed + bb_rate + so_rate*power
m <- polr(player_model,data=player_pairs,weights=pf,Hess=T)
AIC(m)

player_model <- class ~ age + power + speed + bb_rate + so_rate
m <- polr(player_model,data=player_pairs,weights=pf,Hess=T)
AIC(m)

player_model <- class ~ age*speed + power + bb_rate + so_rate
m <- polr(player_model,data=player_pairs,weights=pf,Hess=T)
AIC(m)
